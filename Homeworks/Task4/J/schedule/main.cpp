...
//  schedule
//
//  Created by Дмитрий Ткаченко on 13/10/2016.
...
int main() {
    ...
    mcf.init();
    for (int i = 0; i < N; i++) {
        mcf.add_edge(2+i*2, 2+i*2+1, 1, -X[i].p);
    }
    for (int i = 0; i < N; i++) {
    ...
    for (int i = 0; i < N; i++) {
        mcf.add_edge(0, 2+i*2, 1, 0);
    }
    for (int i = 0; i < N; i++) {
        mcf.add_edge(2+i*2+1, 1, 1, 0);
    }
    mcf.ready(0, 1);
    for (int i = 0; i < N; i++) {
